...
function StaffList() {
  ...
  const handleEdit = (item: Staff) => {
    ...
    setIsEditing(true);
    setShowForm(true);
    setPopup({ visible: false, type: "", staff: item });
  };
  ...
  const uploadImageForId = async (id: number, file: File): Promise<boolean> => {
      ...
      if (isEditing && popup.staff) {
        ...
        if (response?.data?.data) {
          ...
          if (Array.isArray(respData)) {
            respData.forEach((s: { id?: number }) => {
              if (s?.id) createdIds.push(s.id);
            });
          } else if (respData?.id) {
  ...
  const makeImageUrl = (img: string | null | undefined): string | null => {
    if (!img) return null;
      ...
      {showForm && (
        <div className="flex justify-center bg-card rounded-lg mb-4 items-center max-w-6xl">
          <div className="w-full p-6 rounded-lg shadow-md">
            <h1 className="text-2xl font-bold text-center mb-6">{isEditing ? "Edit Staff" : "Add Staff"}</h1>
              ...
              <div className="grid md:grid-cols-2 gap-4 md:gap-6 grid-cols-1">
                ...
                <div>
                  ...
                  {image ? (
                    ...
                    <div className="block mt-2">
                      <img
                        src={makeImageUrl(popup.staff.image) as string}
                        alt="Uploaded"
                        className="w-32 h-32 object-cover rounded-md"
                      />
                      <div className="flex gap-2 mt-2">
