...
type Registration = Omit<
  Class8RegistrationRecord,
  'class8_year' | 'birth_date' | 'created_at' | 'status'
> & {
  class8_year: number;
  birth_date: string;
  created_at: string;
  ...
  const handleExport = useCallback(
    async (type: 'sheet' | 'photos') => {
      ...
      try {
        ...
        downloadBlob(blob, `Class8_${type}_${year}${section ? `_${section}` : ''}.${extension}`);
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    downloadBlob(
                      blob,
                      `Class8_Registration_${selectedReg.student_name_en.replace(/\s+/g, '_')}.pdf`,
                    );
                  } catch (err) {
                    console.error(err);
