...
const SSCRegForm = () => {
  ...
  const fetchSSCReg = async (): Promise<void> => {
    try {
      setFormLoading(true);
  ...
  const handleEditSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      const response = await axios.put(
        `/api/reg/ssc/form/${(editFormData as Partial<Registration>).id}/status`,
        {
          status: (editFormData as Partial<Registration>).status,
        }
      );
      ...
      if (response.data.success) {
        setAllRegistrations((prev) =>
          prev.map((reg) =>
            reg.id === (editFormData as Partial<Registration>).id
              ? { ...reg, status: (editFormData as Partial<Registration>).status || "pending" }
              : reg
          )
        );
  ...
  const confirmDelete = (registration: Registration): void => {
  ...
  const handleExportImages = async (): Promise<void> => {
    try {
      setLoading(true);
  ...
  const getStatusBadge = (status: string | null): React.ReactElement => {
    ...
    if (s === "approved") {
      ...
      classes = `${classes} bg-muted text-gray-800`;
    }
  ...
  const handleFormSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      formDataToSend.append("ssc_year", formData.ssc_year.toString());
      formDataToSend.append("reg_open", formData.reg_open.toString());
      ...
      if (response.data.success) {
        ...
        const fileInput = document.getElementById("notice") as HTMLInputElement;
      ...
      setFormMessage(
        (error as { response?: { data?: { message?: string } } }).response?.data?.message || "Error saving SSC registration"
      );
      setTimeout(() => setFormMessage(""), 3000);
    } finally {
      setFormLoading(false);
  ...
  const removeNotice = async (): Promise<void> => {
    try {
      setFormLoading(true);
      ...
      setFormMessage((error as { response?: { data?: { message?: string } } }).response?.data?.message || "Error removing notice");
      setTimeout(() => setFormMessage(""), 3000);
    } finally {
      setFormLoading(false);
    }
      ...
      <div className="mb-8">
        ...
        <div className="flex justify-end mb-4 md:mt-0">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-700 dark:bg-primary dark:hover:bg-primary/90 transition-colors"
          >
            <RefreshCw size={16} />
            Refresh
          </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border text-input dark:dark:bg-accent border-border rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-blue-500 transition-colors"
                    required
                  />
                </div>
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-border p-6 mb-6 shadow-sm dark:bg-gray-800 dark:border-gray-700">
            <div className="flex items-center gap-2 mb-4">
              <Filter size={18} className="text-muted-foreground dark:text-gray-300" />
                      ...
                      <table className="w-full text-sm">
                        <tbody>
                          ...
                          <tr className="border-b bg-muted/50">
                            ...
                            <td className="py-2 px-4">
                              {(() => {
                                const joinAddr = (village: string, postOffice: string, postCode: string, upazila: string, district: string): string | null => {
                                  return (
                                    [
                                      village || "",
                                      postOffice
                            ...
                            <td className="py-2 px-4">
                              {(() => {
                                const stipend = selectedRegistration.upobritti
                                  ? `উপবৃত্তি: ${selectedRegistration.upobritti === "Yes"
                                    ? "হ্যাঁ"
                                    : "না"
                                  }`
                                  : "";
                                const govScholarship =
                                  selectedRegistration.sorkari_brirti
                                    ? `সরকারি বৃত্তি: ${selectedRegistration.sorkari_brirti ===
                                      "No"
                                      ? "না"
                                      : selectedRegistration.sorkari_brirti ===
                                        "Talentpool"
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          a.href = url;
                          a.download = `SSC_Registration_${selectedRegistration.student_name_en ||
