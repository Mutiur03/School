...
function getUserIdListFromSettings(
  ...
  if (!settings) return [];
  const cls = String(admissionClass || "")
    .trim()
    .toLowerCase();
...
function AdmissionForm() {
  ...
  const admissionUserIdRef = useRef<string | undefined>(
    initialFormState.admissionUserId,
  );
    ...
    const initializeData = async () => {
      try {
        ...
        if (admissionStatusResponse.data) {
          ...
          const lTypeList = parseCsvString(
            admissionStatusResponse.data.list_type,
          );
          const uidList = getUserIdListFromSettings(
            admissionStatusResponse.data,
          );
          const serialRaw = parseCsvString(
            admissionStatusResponse.data.serial_no,
          );
        ...
        if (isEditMode && id) {
          ...
          if (response.data.success) {
            ...
            const resolvedFather = resolveProfessionOnLoad(
              data.father_profession,
              data.father_profession_other,
              fatherOptions,
            );
            const resolvedMother = resolveProfessionOnLoad(
              data.mother_profession,
              data.mother_profession_other,
              motherOptions,
            );
            ...
            const formData = {
              ...
              admissionUserId:
                data.admission_user_id || data.admissionUserId || "",
  ...
  useEffect(() => {
    ...
    const list = getUserIdListFromSettings(
      admissionSettings,
      form.admissionClass,
    );
    ...
    const cls = String(form.admissionClass || "")
      .trim()
      .toLowerCase();
    ...
    if (
      form.listType &&
      listTypeTokens.length > 0 &&
      !listTypeTokens.includes(String(form.listType))
    ) {
    ...
    if (
      form.serialNo &&
      serialList.length > 0 &&
      !serialList.includes(String(form.serialNo))
    ) {
    ...
    if (
      ...
      !user_id_list.includes(String(form.admissionUserId))
    ) {
      dispatch({ type: "SET_FIELD", name: "admissionUserId", value: "" });
    }
  ...
  function resolveProfessionOnLoad(
    value?: string | null,
    ...
    const matched = allowedOptions.find(
      (o) => String(o).trim().toLowerCase() === raw.toLowerCase(),
    );
  ...
  function validate() {
    ...
    if (!form.admissionClass)
      e.admissionClass = "Please select class for admission";
    ...
    if (!form.admissionUserId) {
      ...
      !userIdOptions.includes(form.admissionUserId)
    ) {
    ...
    if (
      ...
      !String(form.registrationNo || "").trim()
    ) {
      e.registrationNo =
        "Registration number is required for class 8 and 9 students";
    }
    if (
      isClassEightOrNine(form.admissionClass) &&
      String(form.registrationNo || "").trim() &&
      !/^\d{10}$/.test(form.registrationNo || "")
    ) {
    ...
    if (form.whatsappNumber && !/^[0-9]{11}$/.test(form.whatsappNumber))
      e.whatsappNumber = "Enter a valid mobile number (exactly 11 digits)";
    ...
    if (!form.sectionInprevSchool.trim())
      e.sectionInprevSchool = "Section in previous school is required";
    if (!form.rollInprevSchool.trim())
      e.rollInprevSchool = "Roll in previous school is required";
    if (!form.prevSchoolPassingYear.trim())
      e.prevSchoolPassingYear = "Passing year is required";
    ...
    if (!form.father_profession.trim()) {
      ...
      !form.father_profession_other?.trim()
    ) {
    ...
    if (!form.mother_profession.trim()) {
      ...
      !form.mother_profession_other?.trim()
    ) {
      e.mother_profession_other = "Please specify mother's profession";
    }
    if (!form.parent_income.trim())
      e.parent_income = "Parent's income is required";
    setErrors(e);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!valid) {
      setTimeout(() => {
        ...
        if (errorFields.length > 0) {
          ...
          if (!firstErrorField) {
            firstErrorField =
              formRef.current?.querySelector(`[name="${errorFields[0]}"]`) ||
              null;
          }
    ...
    try {
      ...
      formData.append(
        "prevSchoolPassingYear",
        form.prevSchoolPassingYear || "",
      );

      formData.append("sectionInprevSchool", form.sectionInprevSchool || "");
      formData.append("rollInprevSchool", form.rollInprevSchool || "");

      const fatherProfessionVal = isOtherProfession(form.father_profession)
        ? form.father_profession_other || ""
        : form.father_profession || "";
      const motherProfessionVal = isOtherProfession(form.mother_profession)
        ? form.mother_profession_other || ""
      ...
      if (form.admissionClass)
        formData.append("admissionClass", form.admissionClass);
      if (form.listType) formData.append("listType", form.listType);
      if (form.admissionUserId)
        formData.append("admissionUserId", form.admissionUserId);
          ...
          <div className="border rounded-lg p-3 sm:p-4 lg:p-6 bg-white shadow-md flex flex-col gap-y-2">
            ...
            >
              ...
              >
                ...
                <option value="(DIS) ">
                  বিশেষ চাহিদা সম্পন্ন ছাত্র (DIS){" "}
                </option>
        ...
        <section className="mb-4 sm:mb-6">
          ...
          <div className="border rounded-lg p-3 sm:p-4 lg:p-6 bg-white shadow-md flex flex-col gap-y-2">
            ...
            <FieldRow
              label="Passing Year :"
              required
              error={errors.prevSchoolPassingYear}
            ...
            <FieldRow
              label="Section :"
              required
              error={errors.sectionInprevSchool}
        ...
        <section className="mb-4 sm:mb-6">
          ...
          <div className="border rounded-lg p-3 sm:p-4 lg:p-6 bg-white shadow-md flex flex-col gap-y-2">
            ...
            <FieldRow
              ...
              required
              error={errors.mother_profession || errors.mother_profession_other}
