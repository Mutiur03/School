...
function ConfirmationAdmission() {
    ...
    const joinAddr = (village?: string | null, postOffice?: string | null, postCode?: string | null, upazila?: string | null, district?: string | null) => {
        return [
            village ?? '',
    ...
    const renderOptionalRow = (label: string, value: string | number | boolean | null | undefined) => {
    ...
    const formatPreviousSchoolMeta = () => {
        ...
        const parts: string[] = [];
        if (admission.section_in_prev_school && String(admission.section_in_prev_school).trim() !== '') {
        ...
        if (admission.prev_school_passing_year && String(admission.prev_school_passing_year).trim() !== '') {
    ...
    const formatGuardianInfo = () => {
        if (!admission) return null;
        if (!admission.guardian_name && !admission.guardian_phone && !admission.guardian_relation && !admission.guardian_nid) {
            return null;
        }
        return [
                    ...
                    <div className="bg-white shadow rounded-b overflow-hidden">
                        <div className="p-8 space-y-8">
                            <div className="text-center">
                                ...
                                <h2 className="text-2xl font-bold text-gray-800 mb-2">Download Your Admission Form</h2>
        ...
        <div className="w-full min-h-screen bg-gray-100 py-8 px-4">
            <div className={`max-w-4xl mx-auto transition-all duration-1000 ${isConfirmed ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'}`}>
                <div className="bg-gray-800 text-white p-6 sm:p-8 rounded-t">
                    <h1 className="text-2xl sm:text-3xl font-bold">Admission Confirmation</h1>
                ...
                <div className="bg-white p-4 sm:p-8 space-y-8">
                    <div className="text-sm font-medium text-gray-800 border border-gray-200 rounded px-3 py-2 bg-gray-50 flex flex-wrap gap-x-4 gap-y-1">
                        ...
                        {admission.admission_user_id ? <span>User ID: {admission.admission_user_id}</span> : null}
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm table-auto md:table-fixed" style={{ minWidth: '600px' }}>
                                    <tbody>
                                        ...
                                        {renderOptionalRow("Father's Name:", admission.father_name_en ? admission.father_name_en.toUpperCase() : undefined)}
                                        {renderOptionalRow("Father's National ID Number:", admission.father_nid)}
                                        {renderOptionalRow("মাতার নাম:", admission.mother_name_bn)}
                                        {renderOptionalRow("Mother's Name:", admission.mother_name_en ? admission.mother_name_en.toUpperCase() : undefined)}
                                        {renderOptionalRow("Mother's National ID Number:", admission.mother_nid)}
                                        {renderOptionalRow("Permanent Address:", joinAddr(
                                        ...
                                        {renderOptionalRow("Present Address:", joinAddr(
                                        ...
                                        {renderOptionalRow("Parent's Annual Income:", formatParentIncome(admission.parent_income))}
