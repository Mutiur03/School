...
const SectionHeader: React.FC<{ step: number; title: string }> = ({
  step,
  title,
}) => (
  <div className="flex items-center gap-2 sm:gap-3 mb-3">
    <span className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-blue-600 text-white font-bold text-sm sm:text-lg shadow">
      {step}
    </span>
...
function Registration() {
  ...
  const [prevSchoolOption, setPrevSchoolOption] = useState(
    schoolConfig.name.en,
  ); // Add state to manage dropdown selection
      ...
      try {
        ...
        if (isEditMode && id) {
          ...
          if (response.data.success) {
            ...
            if (data.prev_school_name === schoolConfig.name.en) {
              setPrevSchoolOption(schoolConfig.name.en);
              dispatch({
                type: "SET_FIELD",
                name: "prevSchoolName",
                value: schoolConfig.name.en,
              });
            } else {
            ...
            const formData = {
              ...
              guardianAddressSameAsPermanent:
                data.guardian_address_same_as_permanent || false,
            ...
            setTimeout(() => {
              ...
              if (data.present_district) {
                const presentUpazilas = getUpazilasByDistrict(
                  data.present_district,
                );
                setPresentUpazillas(presentUpazilas);
              }
              if (data.permanent_district) {
                const permanentUpazilas = getUpazilasByDistrict(
              ...
              if (data.guardian_district) {
                const guardianUpazilas = getUpazilasByDistrict(
                  data.guardian_district,
                );
            ...
            if (
              data.permanent_district === data.guardian_district &&
  ...
  const jscPassingYears = Array.from({ length: 3 }, (_, i) =>
  ...
  useEffect(() => {
    if (form.birthRegNo && form.birthRegNo.length >= 4) {
      ...
      if (/^\d{4}$/.test(year) && years.includes(year)) {
        ...
        dispatch({
          type: "SET_FIELDS",
          fields: { birthYear: "", birthMonth: "", birthDay: "" },
        });
      }
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setErrors((prev) => ({
        ...prev,
        [name]: "File must be smaller than 2MB",
      }));
      return;
    }
  ...
  function validate() {
    ...
    if (!form.prevSchoolName.trim())
      e.prevSchoolName = "Previous school name is required";
    if (!form.prevSchoolDistrict.trim())
      e.prevSchoolDistrict = "Previous school district is required";
    if (!form.prevSchoolUpazila.trim())
      e.prevSchoolUpazila = "Previous school upazila/thana is required";
    if (!form.jscPassingYear.trim())
      e.jscPassingYear = "JSC/JDC passing year is required";
    if (!jscPassingYears.includes(form.jscPassingYear))
      e.jscPassingYear = "Please select a valid JSC/JDC passing year";
    if (!form.jscBoard.trim()) e.jscBoard = "JSC/JDC board is required";
    if (form.jscRegNo.trim() && !/^\d{10}$/.test(form.jscRegNo))
      e.jscRegNo = "JSC/JDC registration number must be exactly 10 digits";
    ...
    if (!form.studentNameEn.trim())
      e.studentNameEn = "Student name (in English) is required";
    if (!form.studentNameBn.trim())
      e.studentNameBn = "ছাত্রের নাম (বাংলায়) is required";
    if (!form.studentNickNameBn?.trim())
      e.studentNickNameBn = "ডাকনাম (এক শব্দে/বাংলায়) is required";
    if (!form.fatherNameEn.trim())
      e.fatherNameEn = "Father's name (in English) is required";
    if (!form.fatherNameBn.trim())
      e.fatherNameBn = "পিতার নাম (বাংলায়) is required";
    if (!form.motherNameEn.trim())
      e.motherNameEn = "Mother's name (in English) is required";
    if (!form.motherNameBn.trim())
      e.motherNameBn = "মাতার নাম (বাংলায়) is required";
    if (!form.birthRegNo.trim())
      e.birthRegNo = "Birth registration number is required";
    if (!/^\d{17}$/.test(form.birthRegNo))
      e.birthRegNo = "Birth registration number must be exactly 17 digits";
    ...
    if (!form.presentDistrict.trim())
      e.presentDistrict = "Present district is required";
    if (!form.presentUpazila.trim())
      e.presentUpazila = "Present upazila/thana is required";
    if (!form.presentPostOffice.trim())
      e.presentPostOffice = "Present post office is required";
    if (!form.presentPostCode.trim())
      e.presentPostCode = "Present post code is required";
    else if (!/^\d{4}$/.test(form.presentPostCode))
      e.presentPostCode = "Present post code must be exactly 4 digits";
    if (!form.presentVillageRoad.trim())
      e.presentVillageRoad = "Present village/road/house is required";
    if (!form.permanentDistrict.trim())
      e.permanentDistrict = "Permanent district is required";
    if (!form.permanentUpazila.trim())
      e.permanentUpazila = "Permanent upazila/thana is required";
    if (!form.permanentPostOffice.trim())
      e.permanentPostOffice = "Permanent post office is required";
    if (!form.permanentPostCode.trim())
      e.permanentPostCode = "Permanent post code is required";
    else if (!/^\d{4}$/.test(form.permanentPostCode))
      e.permanentPostCode = "Permanent post code must be exactly 4 digits";
    if (!form.permanentVillageRoad.trim())
      e.permanentVillageRoad = "Permanent village/road/house is required";
    ...
    if (!form.sorkari_brirti.trim())
      e.sorkari_brirti = "সরকারি বৃত্তি is required";
    if (guardianNotFather) {
      if (!form.guardianName?.trim())
        e.guardianName = "Guardian's name is required";
      if (!form.guardianRelation?.trim())
        e.guardianRelation = "Relationship with guardian is required";
      ...
      if (!form.guardianAddressSameAsPermanent) {
        if (!form.guardianDistrict?.trim())
          e.guardianDistrict = "Guardian's district is required";
        if (!form.guardianUpazila?.trim())
          e.guardianUpazila = "Guardian's upazila/thana is required";
        if (!form.guardianPostOffice?.trim())
          e.guardianPostOffice = "Guardian's post office is required";
        if (!form.guardianPostCode?.trim())
          e.guardianPostCode = "Guardian's post code is required";
        else if (!/^\d{4}$/.test(form.guardianPostCode))
          e.guardianPostCode = "Guardian's post code must be exactly 4 digits";
        if (!form.guardianVillageRoad?.trim())
          e.guardianVillageRoad = "Guardian's village/road/house is required";
      }
    }

    if (!form.groupClassNine)
      e.groupClassNine = "Group in class nine is required";
    if (!form.mainSubject) e.mainSubject = "Main subject is required";
    if (!form.fourthSubject) e.fourthSubject = "4th subject is required";
    if (!form.nearbyNineStudentInfo.trim())
      e.nearbyNineStudentInfo = "This field is required";
    if (!form.sectionInClass8.trim())
      e.sectionInClass8 = "Section in Class Eight is required";
    if (!form.rollInClass8.trim())
      e.rollInClass8 = "Roll in Class Eight is required";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (guardianNotFather) {
        ...
        formData.append(
          "guardianAddressSameAsPermanent",
          form.guardianAddressSameAsPermanent?.toString() || "false",
        );
    ...
    <div className="max-w-full sm:max-w-2xl md:max-w-3xl lg:max-w-4xl xl:max-w-5xl mx-auto px-3 sm:px-4 lg:px-6 py-3 sm:py-4 lg:py-6">
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-blue-100 mb-4 py-2 sm:py-3 px-3 sm:px-4 rounded-t shadow-sm flex flex-col items-center">
        <h2 className="text-xl sm:text-2xl lg:text-3xl text-center font-bold text-blue-700 tracking-tight underline underline-offset-4 mb-1 sm:mb-2">
          {isEditMode
            ? "Edit Registration"
            : `Student's Information for Registration of SSC Exam ${ssc_batch}`}
        </h2>
      ...
      >
        <section className="mb-4 sm:mb-6">
          ...
          <div className="border rounded-lg p-3 sm:p-4 lg:p-6 bg-white shadow-md flex flex-col gap-y-2">
            <FieldRow
              ...
              tooltip="Choose the section you want to be assigned to for SSC"
            >
              <select
                name="section"
                value={form.section}
            ...
            <FieldRow
              ...
              required
              error={errors.roll}
              tooltip="Select your roll number from the available rolls for your chosen section"
            >
              <select
                name="roll"
                value={form.roll}
              ...
              {form.section && availableRolls.length === 0 && (
                <Instruction>
                  No rolls have been configured for section {form.section}.
            ...
            <FieldRow
              ...
              tooltip="Select your religion as it will appear on your SSC certificate"
            >
              <select
                name="religion"
                value={form.religion}
            ...
            <FieldRow
              label="ছাত্রের নাম (বাংলায়):"
              required
              instruction="(জেএসসি/জেডিসি রেজিস্ট্রেশন কার্ড অনুযায়ী)"
              error={errors.studentNameBn}
              tooltip="Enter your name exactly as it appears in your JSC/JDC certificate in Bengali"
            >
              <input
                name="studentNameBn"
                ...
                aria-invalid={!!errors.studentNameBn}
              />
            </FieldRow>
            <FieldRow
              label="ডাকনাম (এক শব্দে/বাংলায়):"
              required
              error={errors.studentNickNameBn}
            ...
            <FieldRow
              ...
              required
              instruction="(According to JSC/JDC Registration Card)"
              error={errors.studentNameEn}
              tooltip="Enter your name exactly as it appears in your JSC/JDC certificate in English capital letters"
            >
              <input
                name="studentNameEn"
                ...
                aria-invalid={!!errors.studentNameEn}
              />
            </FieldRow>
            <FieldRow
              label="Birth Registration Number:"
              required
              error={errors.birthRegNo}
            ...
            <FieldRow
              label="Date of Birth:"
              required
              error={errors.birthYear || errors.birthMonth || errors.birthDay}
            ...
            >
              ...
              <Instruction>
                Student must be at least 12 years old on 1st January{" "}
                {currentYear}.
              </Instruction>
            </FieldRow>
            <FieldRow
              ...
              tooltip="Enter father's name exactly as it appears in JSC/JDC certificate in Bengali"
            >
              <input
                name="fatherNameBn"
                ...
                aria-invalid={!!errors.fatherNameBn}
              />
            </FieldRow>
            <FieldRow
              ...
              tooltip="Enter father's name exactly as it appears in JSC/JDC certificate in English capital letters"
            >
              <input
                name="fatherNameEn"
                ...
                aria-invalid={!!errors.fatherNameEn}
              />
            </FieldRow>
            ...
            <FieldRow
              ...
              tooltip="Enter mother's name exactly as it appears in JSC/JDC certificate in Bengali"
            >
              <input
                name="motherNameBn"
                ...
                aria-invalid={!!errors.motherNameBn}
              />
            </FieldRow>
            <FieldRow
              ...
              tooltip="Enter mother's name exactly as it appears in JSC/JDC certificate in English capital letters"
            >
              <input
                name="motherNameEn"
                ...
                aria-invalid={!!errors.motherNameEn}
              />
            </FieldRow>
            ...
            <FieldRow
              label="Blood Group:"
              tooltip="Select your blood group if known. This is optional but helpful for medical emergencies"
            >
              <select
                name="bloodGroup"
            ...
            <FieldRow
              label="Email:"
              error={errors.email}
              tooltip="Enter a valid email address for communication. This is optional but recommended"
            >
              <input
                name="email"
                value={form.email}
        ...
        <section className="mb-4 sm:mb-6">
          ...
          <div className="border rounded-lg p-3 sm:p-4 lg:p-6 bg-white shadow-md flex flex-col gap-y-2">
            ...
            <FieldRow
              label="Post Code:"
              required
              error={errors.permanentPostCode}
            ...
            <FieldRow
              ...
              required
              error={errors.permanentVillageRoad}
            ...
            <h4 className="font-semibold mb-2 mt-4 sm:mt-6 text-sm sm:text-base">
              Present Address:
            </h4>
            <FieldRow
              label="Same as Permanent:"
              tooltip="Check this box if your present address is the same as permanent address"
            >
            ...
            {!sameAddress && (
              <div className="space-y-2">
                ...
                <FieldRow
                  label="Post Office:"
                  required
                  error={errors.presentPostOffice}
                >
                  <input
                ...
                <FieldRow
                  label="Village/Road/House No:"
                  required
                  error={errors.presentVillageRoad}
                >
                  <input
        ...
        <section className="mb-4 sm:mb-6">
          <SectionHeader
            step={3}
            title="Guardian Information (if not father):"
          />
          <div className="border rounded-lg p-3 sm:p-4 lg:p-6 bg-white shadow-md flex flex-col gap-y-2">
            <FieldRow
                ...
                {!form.guardianAddressSameAsPermanent && (
                  <div className="space-y-2">
                    ...
                    <FieldRow
                      label="Post Office:"
                      required
                      error={errors.guardianPostOffice}
                    ...
                    <FieldRow
                      label="Village/Road/House No:"
                      required
                      error={errors.guardianVillageRoad}
              ...
              <div>
                <div className="flex flex-col lg:flex-row items-start gap-4">
                    ...
                    <div className="relative w-32 h-32 sm:w-40 sm:h-40 lg:w-48 lg:h-48 border-2 border-dashed rounded-lg flex items-center justify-center text-gray-400 bg-gray-50 overflow-hidden">
                      {photoPreview ? (
                        ...
                        <div className="text-center px-2">
                          <svg
                            className="mx-auto mb-1 w-6 h-6 text-gray-300"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                            <path
                              ...
                              strokeWidth="2"
                              d="M3 8l7-5 7 5v9a2 2 0 01-2 2H5a2 2 0 01-2-2V8z"
                            />
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <Instruction>
                      JPG only. Max file size 2MB. Click the box or "Choose
                      Photo" to upload.
                    </Instruction>
                    ...
                    {isEditMode && !form.photo && photoPreview && (
                      <Instruction>
                        Existing photo will be kept if you don't upload a new
                        one.
                      </Instruction>
                    )}
                  </div>
        ...
        <section className="mb-4 sm:mb-6">
          ...
          <div className="border rounded-lg p-3 sm:p-4 lg:p-6 bg-white shadow-md flex flex-col gap-y-2">
            <FieldRow
              label="Name of Previous School (Class 8):"
              required
              error={errors.prevSchoolName}
              tooltip="Select your previous school or choose 'Others' to type the name"
            >
              <select
            ...
            <FieldRow
              label="District:"
              required
              error={errors.prevSchoolDistrict}
            ...
            <FieldRow
              label="Upazila/Thana:"
              required
              error={errors.prevSchoolUpazila}
        ...
        <section className="mb-4 sm:mb-6">
          ...
          <div className="border rounded-lg p-3 sm:p-4 lg:p-6 bg-white shadow-md flex flex-col gap-y-2">
            <FieldRow
              ...
              required
              error={errors.sectionInClass8}
              tooltip="Select the section you were in during class Eight"
            >
              <select
            ...
            <FieldRow
              ...
              required
              error={errors.rollInClass8}
              tooltip="Enter your roll number in class Eight"
            >
              <input
                name="rollInClass8"
            ...
            <div className="flex flex-col md:flex-row gap-2">
              <div className="flex-1">
                <FieldRow
                  ...
                  required
                  error={errors.jscPassingYear}
                  tooltip="Select the year when you passed JSC/JDC examination"
                >
                  <select
                    name="jscPassingYear"
              ...
              <div className="flex-1">
                <FieldRow
                  label="JSC/JDC Board:"
                  required
                  error={errors.jscBoard}
                  tooltip="Select the education board from which you passed JSC/JDC"
                >
                  <select
                    name="jscBoard"
                    value={form.jscBoard}
            ...
            <div className="flex flex-col md:flex-row gap-2">
              <div className="flex-1">
                <FieldRow
                  ...
                  error={errors.jscRegNo}
                  tooltip="Enter your JSC/JDC registration number (exactly 10 digits)"
                >
                  <input
                    name="jscRegNo"
                    value={form.jscRegNo}
              ...
              <div className="flex-1">
                <FieldRow
                  ...
                  error={errors.jscRollNo}
                  tooltip="Enter your JSC/JDC roll number if available (6 digits). This field is optional"
                >
                  <input
                    name="jscRollNo"
        ...
        <section className="mb-4 sm:mb-6">
          ...
          <div className="border rounded-lg p-3 sm:p-4 lg:p-6 bg-white shadow-md flex flex-col gap-y-2">
            <FieldRow
              label="Group in Class Nine:"
              required
              error={errors.groupClassNine}
              tooltip="Select the group you studied in class nine (Science or Humanities)"
            >
              <select
                name="groupClassNine"
              ...
              >
                ...
                {form.groupClassNine &&
                  subjectOptionsByGroup[
                    form.groupClassNine as keyof typeof subjectOptionsByGroup
                  ]?.main.map((opt) => (
            ...
            <FieldRow
              ...
              required
              error={errors.nearbyNineStudentInfo}
              tooltip="Select a current class nine student from your neighborhood area for reference"
            >
              <>
                <select
            ...
            <FieldRow
              label="উপবৃত্তি পায় কিনা:"
              required
              error={errors.upobritti}
              tooltip="Select whether you receive any stipend."
            >
              <select
                name="upobritti"
            ...
            <FieldRow
              label="সরকারি বৃত্তি পায় কিনা:"
              required
              error={errors.sorkari_brirti}
              tooltip="Select whether you receive any government scholarship"
            >
              <select
                name="sorkari_brirti"
