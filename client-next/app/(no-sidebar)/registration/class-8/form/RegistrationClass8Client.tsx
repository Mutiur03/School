...
import type { Class8RegistrationSettings } from '@/queries/registration.queries';
  ...
  const onSubmit = async (data: Class8Registration) => {
    ...
    try {
      ...
      if (response.data.success) {
        router.push(`/registration/class-8/confirm/${response.data.data.id}`);
      }
    } catch (error: any) {
    ...
    <div className="mx-auto max-w-full px-3 py-3 sm:max-w-2xl sm:px-4 sm:py-4 md:max-w-3xl lg:max-w-4xl lg:px-6 lg:py-6 xl:max-w-5xl">
      ...
      >
        <SectionHeader title="Personal Information">
          ...
          <FieldRow label="Scout:" isRequired error={errors.scout_status}>
            <select
