...
import DuplicateWarning, {
  Duplicate,
} from "@/components/Form/DuplicateWarning";
...
const registrationMetadata = {
  ...
  student_name_en: {
    tooltip:
      "Write student's name in English (Capital Letters) as per JSC/JDC certificate",
    instruction:
      "Write Student's Name in English (Capital Letters) as per JSC/JDC certificate",
  },
  birth_reg_no: {
  ...
  father_name_en: {
    tooltip:
      "Write father's name in English (Capital Letters) as per JSC/JDC certificate",
    instruction:
      "Write Father's Name in English (Capital Letters) as per JSC/JDC certificate",
  },
  father_nid: {
  ...
  mother_name_en: {
    tooltip:
      "Write mother's name in English (Capital Letters) as per JSC/JDC certificate",
    instruction:
      "Write Mother's Name in English (Capital Letters) as per JSC/JDC certificate",
  },
  mother_nid: {
  ...
  photo: {
    ...
    instruction:
      "বিদ্যালয় ইউনিফর্ম পরিহিত রঙ্গিন ছবি আপলোড করুন (Portrait 15:19 ratio)",
  },
};
...
export default function RegistrationClass9Client({
  ...
  const [initialPermanentUpazila, setInitialPermanentUpazila] = useState<
    string | null
  >(null);
  const [initialPresentUpazila, setInitialPresentUpazila] = useState<
    string | null
  >(null);
  const [initialPrevSchoolUpazila, setInitialPrevSchoolUpazila] = useState<
    string | null
  >(null);
  ...
  const nearby_nine_student_info = useWatch({
    control,
    name: "nearby_nine_student_info",
  });
    ...
    const initializeData = () => {
      try {
        ...
        if (isEditMode && id) {
          ...
          setValue("prev_school_district", schoolConfig.contact.district, {
            shouldValidate: true,
          });
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (
      ...
      nearbyOptions.includes(nearby_nine_student_info)
    ) {
    ...
    if (hasOptions) {
      const timer = setTimeout(() => {
        if (initialPermanentUpazila)
          setValue("permanent_upazila", initialPermanentUpazila, {
            shouldValidate: true,
          });
        if (initialPresentUpazila)
          setValue("present_upazila", initialPresentUpazila, {
            shouldValidate: true,
          });
        if (initialPrevSchoolUpazila)
          setValue("prev_school_upazila", initialPrevSchoolUpazila, {
            shouldValidate: true,
          });
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (!currentUpazila || currentUpazila === "") {
        ...
        if (exists) {
          setValue("prev_school_upazila", targetUpazila, {
            shouldValidate: true,
          });
        }
      }
    ...
    if (file) {
      ...
      img.onload = () => {
        ...
        if (Math.abs(ratio - targetRatio) > tolerance) {
          alert(
            "Image aspect ratio MUST be 15:19 (Portrait). Please resize your image.",
          );
          e.target.value = "";
          return;
        }
    ...
    try {
      ...
      if (data.photo instanceof File) {
        const { data: uploadData } = await axios.post(
          "/api/reg/class-9/form/upload-url",
          {
      ...
      const endpoint = isEditMode
        ...
        : "/api/reg/class-9/form";
      const method = isEditMode ? "put" : "post";
      const response = await axios[method](endpoint, submissionData);
      if (response.data.success) {
        router.push(`/registration/class-9/confirm/${response.data.data.id}`);
      }
    } catch (error: any) {
    ...
    <div className="max-w-full sm:max-w-2xl md:max-w-3xl lg:max-w-4xl xl:max-w-5xl mx-auto px-3 sm:px-4 lg:px-6 py-3 sm:py-4 lg:py-6">
      ...
      >
        <SectionHeader title="Personal Information">
          ...
          <FieldRow label="Religion:" isRequired error={errors.religion}>
            <select
              {...register("religion")}
              className="block w-full border rounded px-3 py-2 text-sm sm:text-base transition focus:outline-none focus:ring-2 focus:ring-blue-300"
            >
