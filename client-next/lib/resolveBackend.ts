...
export const serverBackendUrl = () =>
  trimTrailingSlash(
    String(
      process.env.API_URL ?? process.env.BACKEND_URL ?? process.env.NEXT_PUBLIC_BACKEND_URL ?? '',
    ).trim(),
  );

export const TENANT_CLIENT_HOST_SUFFIX =
  process.env.NEXT_PUBLIC_TENANT_CLIENT_HOST_SUFFIX || '-school.mutiurrahman.com';
...
export function getDefaultTenantHost(): string | undefined {
  const value = String(
    process.env.NEXT_PUBLIC_DEFAULT_TENANT_HOST ?? process.env.NEXT_PUBLIC_DEV_TENANT_HOST ?? '',
  ).trim();
