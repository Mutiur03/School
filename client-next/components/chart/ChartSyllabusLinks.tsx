...
export function ChartSyllabusLinks() {
  ...
  const handleSyllabusClick = async (classNum: number) => {
    const syllabuses = syllabusesQuery.data ?? (await syllabusesQuery.refetch()).data ?? [];
    const classSyllabuses = syllabuses.filter((syllabus) => syllabus.class === classNum);
    const latest = classSyllabuses.reduce(
      (currentLatest, syllabus) => (syllabus.year > currentLatest.year ? syllabus : currentLatest),
      classSyllabuses[0],
    );
  ...
  return (
    <ul>
      {[6, 7, 8, 9, 10].map((classNum) => (
        <li key={classNum}>
          <button type="button" onClick={() => handleSyllabusClick(classNum)} className="as-link">
