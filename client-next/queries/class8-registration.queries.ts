...
export const getClass8RegistrationSettings = async (): Promise<Class8RegistrationSettings> => {
  try {
    const response = await api.get<Class8RegistrationSettings>('/api/reg/class-8');
...
export const getClass8RegistrationRecord = async (
  ...
  try {
    const response = await api.get<Class8RegistrationRecord>(`/api/reg/class-8/form/${id}`, {
      cache: 'no-store',
    });
