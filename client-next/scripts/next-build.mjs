...
if (!fs.existsSync(nestedManifest) && fs.existsSync(flatManifest)) {
  ...
  console.log(`[open-next patch] Restructured standalone layout → standalone/${packageDir}/`);
}
...
if (helpersSrc) {
  const helpersDests = new Set(
    uniqueDests.map((nextDest) => path.join(path.dirname(nextDest), '@swc', 'helpers')),
  );
...
if (styledJsxSrc) {
  const styledJsxDests = new Set(
    uniqueDests.map((nextDest) => path.join(path.dirname(nextDest), 'styled-jsx')),
  );
