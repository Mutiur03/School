...
export class TeacherController {
  ...
  static addTeacherController = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=teachers_credentials.xlsx",
      );
  ...
  static saveTeacherImageController = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      const result = await TeacherService.saveTeacherImage(parseInt(id as string), key);
      res
        .status(200)
        .json(
  ...
  static removeTeacherImageController = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      const result = await TeacherService.saveTeacherImage(parseInt(id as string), null);
      res
        .status(200)
        .json(
  ...
  static updateHeadMessageController = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      res
        .status(200)
        .json(new ApiResponse(200, result, "Head message updated successfully"));
    },
  );
  ...
  static getHeadMessageController = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      res
        .status(200)
        .json(new ApiResponse(200, headMsg, "Head message fetched successfully"));
    },
  );
}
