...
export class StudentController {
  static getAlumniController = asyncHandler(async (_req: Request, res: Response) => {
    const students = await StudentService.getAlumni();
    res.status(200).json(new ApiResponse(200, students, 'Alumni fetched successfully'));
  });

  static getStudentsQueryController = asyncHandler(async (req: Request, res: Response) => {
    ...
    const group = typeof groupValue === 'string' ? groupValue : undefined;
  ...
  static getAttendanceOverviewController = asyncHandler(async (req: Request, res: Response) => {
    const yearValue = req.query.year;
    const year = typeof yearValue === 'string' ? parseInt(yearValue, 10) : NaN;
    ...
    const level = typeof levelValue === 'string' ? parseInt(levelValue, 10) : undefined;
    const section = typeof sectionValue === 'string' ? sectionValue : undefined;
    ...
    res.status(200).json(new ApiResponse(200, data, 'Attendance overview fetched successfully'));
  });

  static getMyProfileController = asyncHandler(async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Unauthorized');
    }

    const yearValue = req.query.year;
    const year = typeof yearValue === 'string' ? parseInt(yearValue, 10) : undefined;

    const data = await StudentService.getStudentProfile(req.user.id, year);

    res.status(200).json(new ApiResponse(200, data, 'Profile fetched successfully'));
  });

  static getMyAttendanceController = asyncHandler(async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Unauthorized');
    }
    ...
    const year = typeof yearValue === 'string' ? parseInt(yearValue, 10) : NaN;
    const month = typeof monthValue === 'string' ? parseInt(monthValue, 10) : undefined;
  ...
  static getStudentAttendanceController = asyncHandler(async (req: Request, res: Response) => {
  ...
  static addStudentController = asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await StudentService.addStudents(parsedRequest.data.students);
  ...
  static updateStudentController = asyncHandler(async (req: Request, res: Response) => {
    ...
    const updatedStudent = await StudentService.updateStudent(parsedId.data, parsedUpdates.data);
    res.status(200).json(new ApiResponse(200, updatedStudent, 'Student updated successfully'));
  });

  static deleteStudentController = asyncHandler(async (req: Request, res: Response) => {
    const parsedId = enrollmentIdParamSchema.safeParse(req.params.id);
    if (!parsedId.success) {
      throw new ApiError(400, 'Invalid student id', parsedId.error.issues);
    }
  ...
  static deleteStudentsBulkController = asyncHandler(async (req: Request, res: Response) => {
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, { deletedCount }, `Deleted ${deletedCount} students successfully`),
      );
  });

  static rotatePasswordsBulkController = asyncHandler(async (req: Request, res: Response) => {
    const parsedRequest = rotatePasswordsBulkRequestSchema.safeParse(req.body);
    if (!parsedRequest.success) {
      throw new ApiError(
        400,
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=rotated_passwords.xlsx');

    res.status(200).send(excelBuffer);
  });

  static updateAcademicInfoController = asyncHandler(async (req: Request, res: Response) => {
    const parsedEnrollmentId = enrollmentIdParamSchema.safeParse(req.params.enrollment_id);
    if (!parsedEnrollmentId.success) {
      throw new ApiError(400, 'Invalid enrollment_id parameter', parsedEnrollmentId.error.issues);
    }
    ...
    res
      .status(200)
      .json(new ApiResponse(200, updatedEnrollment, 'Academic info updated successfully'));
  });
  ...
  static saveStudentImageController = asyncHandler(async (req: Request, res: Response) => {
    const parsedId = enrollmentIdParamSchema.safeParse(req.params.id);
    if (!parsedId.success) {
      throw new ApiError(400, 'Invalid student id', parsedId.error.issues);
    }
    ...
    res.status(200).json(new ApiResponse(200, result, 'Student image updated successfully'));
  });

  static changePasswordController = asyncHandler(async (req: Request, res: Response) => {
    if (!req.user) {
      throw new ApiError(401, 'Unauthorized');
    }
  ...
  static generateTestimonialsController = asyncHandler(async (req: Request, res: Response) => {
    const parsedId = enrollmentIdParamSchema.safeParse(req.params.id);
    if (!parsedId.success) {
      throw new ApiError(400, 'Invalid student id', parsedId.error.issues);
    }
  ...
  static giveTransferCertificateController = asyncHandler(async (req: Request, res: Response) => {
    const parsedId = enrollmentIdParamSchema.safeParse(req.params.id);
    if (!parsedId.success) {
      throw new ApiError(400, 'Invalid student id', parsedId.error.issues);
    }
  ...
  static reactivateStudentController = asyncHandler(async (req: Request, res: Response) => {
    const parsedId = enrollmentIdParamSchema.safeParse(req.params.id);
    if (!parsedId.success) {
      throw new ApiError(400, 'Invalid student id', parsedId.error.issues);
    }
    ...
    res.status(200).json(new ApiResponse(200, result, 'Student reactivated successfully.'));
  });
}
