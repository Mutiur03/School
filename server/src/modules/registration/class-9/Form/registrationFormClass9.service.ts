  ...
  try {
    if (data && data.birth_reg_no && data.ssc_batch) {
      ...
      if (existing) {
        duplicates.push({
          field: "birthRegNo",
          message: `এই জন্ম নিবন্ধন নম্বর (Birth Reg No) দিয়ে ${data.ssc_batch} শিক্ষাবর্ষে ইতিমধ্যেই একটি নিবন্ধন বিদ্যমান`,
        });
      }
    }
    ...
    if (data && data.section && data.roll && data.ssc_batch) {
      ...
      if (existing) {
        duplicates.push({
          field: "rollSection",
          message: `${data.ssc_batch} শিক্ষাবর্ষে ${data.section} শাখায় এই রোল নম্বর (${data.roll}) ইতিমধ্যেই নিবন্ধিত`,
        });
      }
    }
  } catch (err) {
...
export class RegistrationFormClass9Service {
  static async getRegistrationPhotoUploadUrl(data: any) {
    ...
    const key = `registrations/class-9/photo-${Date.now()}${ext}`;
  ...
  static async updateRegistration(id: string, data: any) {
    ...
    if (!data.photo) {
      data.photo = existing.photo_path;
    }

    if (data.photo && existing.photo_path && data.photo !== existing.photo_path) {
    ...
    if (data.class9_year || data.ssc_year || data.ssc_batch) {
      data.ssc_batch = String(data.ssc_batch || data.class9_year || data.ssc_year);
    }
  ...
  static async downloadRegistrationPDF(
    id: string,
    previewParam: string = "",
  ): Promise<any> {
    ...
    let qrCodeBase64 = "";
    try {
      const qrData = `${frontendDomain}/preview/class9/${registration.id}`;
    ...
    const studentDetails = [
      ...
      [
        "Student's Name:",
        wrapBnEn(registration.student_name_en || ""),
      ],
      ...
      [
        "Father's Name:",
        wrapBnEn(registration.father_name_en || ""),
      ],
      ...
      [
        "Mother's Name:",
        wrapBnEn(registration.mother_name_en|| ""),
      ],
            ...
            <div class="header">
              <div class="header-top">
                ...
                <div class="header-center">
                  ...
                  <div class="title-row en">Student's Information for Registration of SSC Exam ${class9Year}</div>
                </div>
