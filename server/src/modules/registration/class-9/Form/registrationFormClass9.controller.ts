...
export class RegistrationFormClass9Controller {
  ...
  static getRegistrationById = asyncHandler(async (req: Request, res: Response) => {
    const registration = await RegistrationFormClass9Service.getRegistrationById(
  ...
  static updateRegistration = asyncHandler(async (req: Request, res: Response) => {
    const existing = await RegistrationFormClass9Service.getRegistrationById(
      req.params.id as string,
    );
    assertPendingFormEditAllowed(req, existing.status);
    const updated = await RegistrationFormClass9Service.updateRegistration(
      req.params.id as string,
      req.body,
    );
  ...
  static updateRegistrationStatus = asyncHandler(async (req: Request, res: Response) => {
    ...
    const updated = await RegistrationFormClass9Service.updateRegistrationStatus(
      req.params.id as string,
      req.body.status,
    );
    res
  ...
  static exportRegistrationPhotos = asyncHandler(async (req: Request, res: Response) => {
    const { archive, registrations } = await RegistrationFormClass9Service.exportRegistrationPhotos(
      req.query,
    );
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=Class_9_Photos_${ssc_batch || 'All'}_${section || 'All'}.zip`,
    );
  ...
  static downloadRegistrationPDF = asyncHandler(async (req: Request, res: Response) => {
    const result = await RegistrationFormClass9Service.downloadRegistrationPDF(
      req.params.id as string,
    ...
    res.setHeader(
      'Content-Disposition',
      `${pdfResult.isInlinePreview ? 'inline' : 'attachment'}; filename="${fileName}"`,
    );
