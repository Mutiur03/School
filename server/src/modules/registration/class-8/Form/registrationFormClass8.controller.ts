...
export class RegistrationFormClass8Controller {
  ...
  static getRegistrationById = asyncHandler(async (req: Request, res: Response) => {
    const registration = await RegistrationFormClass8Service.getRegistrationById(
  ...
  static updateRegistration = asyncHandler(async (req: Request, res: Response) => {
    const existing = await RegistrationFormClass8Service.getRegistrationById(
      req.params.id as string,
    );
    assertPendingFormEditAllowed(req, existing.status);
    const updated = await RegistrationFormClass8Service.updateRegistration(
      req.params.id as string,
      req.body,
    );
  ...
  static updateRegistrationStatus = asyncHandler(async (req: Request, res: Response) => {
    ...
    const updated = await RegistrationFormClass8Service.updateRegistrationStatus(
      req.params.id as string,
      req.body.status,
    );
    res
  ...
  static exportRegistrationPhotos = asyncHandler(async (req: Request, res: Response) => {
    const { archive, registrations } = await RegistrationFormClass8Service.exportRegistrationPhotos(
      req.query,
    );
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=Class8_Photos_${class8_year || 'All'}_${section || 'All'}.zip`,
    );
  ...
  static downloadRegistrationPDF = asyncHandler(async (req: Request, res: Response) => {
    const result = await RegistrationFormClass8Service.downloadRegistrationPDF(
      req.params.id as string,
