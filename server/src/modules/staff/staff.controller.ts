...
export class StaffController {
  static getStaffsController = asyncHandler(async (req: Request, res: Response) => {
  ...
  static addStaffController = asyncHandler(async (req: Request, res: Response) => {
    ...
    res.status(201).json(new ApiResponse(201, data, 'Staffs added successfully'));
  });

  static updateStaffController = asyncHandler(async (req: Request, res: Response) => {
    ...
    res.status(200).json(new ApiResponse(200, result, 'Staff updated successfully'));
  });

  static deleteStaffController = asyncHandler(async (req: Request, res: Response) => {
    const id = parseInput(staffIdParamSchema, req.params.id);
    const data = await StaffService.deleteStaff(id);

    res.status(200).json(new ApiResponse(200, data, 'Staff deleted successfully'));
  });

  static getStaffPresignedUrlController = asyncHandler(async (req: Request, res: Response) => {
    const { id, filename, contentType } = parseInput(staffPresignedUrlQuerySchema, req.query);

    const result = await StaffService.getPresignedUploadUrl(id, filename, contentType);

    res.json(new ApiResponse(200, result, 'Presigned URL generated successfully'));
  });

  static saveStaffImageController = asyncHandler(async (req: Request, res: Response) => {
    const id = parseInput(staffIdParamSchema, req.params.id);
    const { key } = parseInput(staffImageKeySchema, req.body);
    const result = await StaffService.saveStaffImage(id, key);

    res.status(200).json(new ApiResponse(200, result, 'Staff image updated successfully'));
  });

  static removeStaffImageController = asyncHandler(async (req: Request, res: Response) => {
    const id = parseInput(staffIdParamSchema, req.params.id);
    const result = await StaffService.saveStaffImage(id, null);

    res.status(200).json(new ApiResponse(200, result, 'Staff image removed successfully'));
  });
}
