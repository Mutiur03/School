...
import {
  teacherFormSchema,
  rotateTeachersPasswordsBulkRequestSchema,
...
export class TeacherController {
  static getTeachersController = asyncHandler(async (req: Request, res: Response) => {
  ...
  static updateTeacherController = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const parsedUpdates = teacherFormSchema.safeParse(req.body);
    if (!parsedUpdates.success) {
      throw new ApiError(
        400,
        parsedUpdates.error.issues[0]?.message || 'Invalid teacher update data',
  ...
  static deleteTeacherController = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
  ...
  static getTeacherImageUploadUrlController = asyncHandler(async (req: Request, res: Response) => {
  ...
  static saveTeacherImageController = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const { key } = req.body;
    const teacherId = parseInt(id as string);
    if (req.user?.role === 'teacher' && req.user.id !== teacherId) {
    ...
    res.status(200).json(new ApiResponse(200, result, 'Teacher image updated successfully'));
  });
  ...
  static saveTeacherSignatureController = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const { key } = req.body;
    const teacherId = parseInt(id as string);
    if (req.user?.role === 'teacher' && req.user.id !== teacherId) {
    ...
    res.status(200).json(new ApiResponse(200, result, 'Teacher signature updated successfully'));
  });

  static removeTeacherSignatureController = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const teacherId = parseInt(id as string);
    if (req.user?.role === 'teacher' && req.user.id !== teacherId) {
      throw new ApiError(403, 'You can only update your own signature');
    ...
    res.status(200).json(new ApiResponse(200, result, 'Teacher signature removed successfully'));
  });

  static removeTeacherImageController = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const teacherId = parseInt(id as string);
    if (req.user?.role === 'teacher' && req.user.id !== teacherId) {
      throw new ApiError(403, 'You can only update your own image');
    ...
    const result = await TeacherService.saveTeacherImage(teacherId, null);
    res.status(200).json(new ApiResponse(200, result, 'Teacher image removed successfully'));
  });
  ...
  static changePasswordController = asyncHandler(async (req: Request, res: Response) => {
    ...
    await TeacherService.changePassword(req.user.id, currentPassword, newPassword);
    res.status(200).json(new ApiResponse(200, null, 'Password changed successfully'));
  });

  static rotatePasswordsBulkController = asyncHandler(async (req: Request, res: Response) => {
    const parsedRequest = rotateTeachersPasswordsBulkRequestSchema.safeParse(req.body);
  ...
  static updateHeadMessageController = asyncHandler(async (req: Request, res: Response) => {
    ...
    res.status(200).json(new ApiResponse(200, result, 'Head message updated successfully'));
  });

  static getHeadMessageController = asyncHandler(async (req: Request, res: Response) => {
    const headMsg = await TeacherService.getHeadMessage(req.schoolId);
    res.status(200).json(new ApiResponse(200, headMsg, 'Head message fetched successfully'));
  });
}
