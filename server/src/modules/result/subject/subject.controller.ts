...
export class SubjectController {
  static addSubController = asyncHandler(async (req: Request, res: Response) => {
    const { subjects } = req.body;
    await SubjectService.addSubjects(subjects);
    res.status(201).json(new ApiResponse(201, null, 'Subjects added successfully'));
  });

  static getSubsController = asyncHandler(async (_req: Request, res: Response) => {
    const subjects = await SubjectService.getSubjects();
    res.status(200).json(new ApiResponse(200, subjects, 'Subjects fetched successfully'));
  });

  static deleteSubController = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
  ...
  static updateSubController = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    res.status(200).json(new ApiResponse(200, result, 'Subject updated successfully'));
  });

  static cloneSubController = asyncHandler(async (req: Request, res: Response) => {
    ...
    res.status(200).json(new ApiResponse(200, result.mapping, 'Subjects cloned successfully'));
  });
}
