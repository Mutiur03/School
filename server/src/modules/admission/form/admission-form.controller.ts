...
export class AdmissionFormController {
  static getAdmissionUploadUrl = asyncHandler(async (req: Request, res: Response) => {
  ...
  static exportAllAdmissionsExcel = asyncHandler(async (req: Request, res: Response) => {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="admissions_export_${new Date().toISOString().slice(0, 10)}.xlsx"`,
    );
    res.status(200).send(buffer);
  });

  static exportAdmissionImagesZip = asyncHandler(async (req: Request, res: Response) => {
    const { archive, forms, fileName } = await AdmissionFormService.exportAdmissionImagesZip(
      req.query,
    );
