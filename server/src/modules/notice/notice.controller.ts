...
export const getPresignedUrlController = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res.status(200).json(new ApiResponse(200, result, 'Presigned URL generated successfully'));
});

export const addNoticeController = asyncHandler(async (req: Request, res: Response) => {
  const { title, key, created_at } = req.body;
  if (!title || !key) {
    throw new ApiError(400, 'title and key are required');
  }

  const result = await service.createNotice({ title, key, created_at }, req.schoolId);
  return res.status(201).json(new ApiResponse(201, result, 'Notice added successfully'));
});

export const getNoticesController = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res.status(200).json(new ApiResponse(200, result, 'Notices fetched successfully'));
});
...
export const updateNoticeController = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res.status(200).json(new ApiResponse(200, result, 'Notice updated successfully'));
});
