...
echo "Starting DB for project: $PROJECT_NAME using env: $ENV_FILE"

# Setup network first
./setup-network.sh "$ENV_FILE"

# Start the container using docker compose
docker compose -p "$PROJECT_NAME" --env-file "$ENV_FILE" -f postgres-run.yml up -d --build

echo "✅ DB container started for $PROJECT_NAME"
