import { z } from 'zod';
import { isValidDob, normalizeText, normalizeOptionalText, normalizeDob } from './utils.js';
import {
  NAME,
  PHONE_NUMBER,
  ROLL_NUMBER,
...
export const studentFormSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (
      ...
      !VALID_GROUPS.includes(value.group as (typeof VALID_GROUPS)[number])
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['group'],
...
export const addStudentInputSchema = z
  .object({
    ...
    father_phone: z.any().refine((value) => typeof value === 'string' && PHONE_NUMBER.test(value), {
...
>;
export type StudentPasswordUpdateData = z.infer<typeof studentPasswordUpdateSchema>;
