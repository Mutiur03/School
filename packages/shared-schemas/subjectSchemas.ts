...
export const subjectFormSchema = z
  .object({
    ...
    class: numericCoerce.pipe(
      z.number().min(1, 'Class must be at least 1').max(12, 'Class must be at most 12'),
    ),
    full_mark: numericCoerce.pipe(z.number().min(0, 'Full mark must be non-negative').nullable()),
  ...
  .superRefine((data, ctx) => {
    ...
    if (totalProvided > 0 && data.full_mark !== undefined && data.full_mark !== totalProvided) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['full_mark'],
    ...
    if (data.group && data.group.trim() !== '') {
      if (!VALID_GROUPS.includes(data.group as (typeof VALID_GROUPS)[number])) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['group'],
    ...
    if (
      data.pass_mark !== null &&
      data.full_mark !== null &&
      Number(data.pass_mark) > Number(data.full_mark)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['pass_mark'],
    ...
    if (data.marking_scheme === 'BREAKDOWN' && data.subject_type !== 'main') {
      ...
      if (totalBreakdown === 0) {
        ctx.addIssue({
          ...
          path: ['cq_mark'],
          message:
            'At least one breakdown mark (CQ, MCQ, or Practical) must be non-zero for BREAKDOWN scheme',
        });
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['mcq_mark'],
      ...
      if (
        data.full_mark !== null &&
        totalBreakdown > 0 &&
        Number(data.full_mark) !== totalBreakdown
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['full_mark'],
      ...
      if (cq_pass > cq) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['cq_pass_mark'],
          message: `CQ Pass (${cq_pass}) cannot exceed CQ Mark (${cq})`,
        });
      }
      if (mcq_pass > mcq) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['mcq_pass_mark'],
          message: `MCQ Pass (${mcq_pass}) cannot exceed MCQ Mark (${mcq})`,
        });
      }
      if (practical_pass > practical) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['practical_pass_mark'],
          message: `Practical Pass (${practical_pass}) cannot exceed Practical Mark (${practical})`,
        });
      }
