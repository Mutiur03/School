...
const admissionResultListTypeSchema = z.enum(['merit_list', 'waiting_list_1', 'waiting_list_2']);
...
const admissionResultUploadFileSchema = z.object({
  ...
  fileSize: z
    ...
    .max(5 * 1024 * 1024 * 1024, 'File size exceeds maximum allowed size'),
  type: admissionResultListTypeSchema,
});

export const admissionResultUploadRequestSchema = z.object({
  files: z.array(admissionResultUploadFileSchema).min(1, 'At least one file is required'),
...
export type AdmissionResultUploadRequestData = z.infer<typeof admissionResultUploadRequestSchema>;
export type AdmissionResultMultipartSignData = z.infer<typeof admissionResultMultipartSignSchema>;
