...
export const registrationObjectShape = z.object({
  ...
  email: z.preprocess(
    ...
    z
      .string()
      .default('')
      .refine((val) => !val || ASCII_ONLY.test(val), 'Email must contain only English characters')
  ...
  guardian_phone: z.preprocess(
    ...
    z
      .string()
      .max(11)
      .refine((val) => !val || PHONE_NUMBER.test(val), 'Invalid Bangladeshi phone number')
      .default(''),
  ),
  ...
  guardian_nid: z.preprocess(
    ...
    z
      .string()
      .max(17)
      .refine((val) => !val || NID.test(val), 'Guardian NID must be 10, 13 or 17 digits')
      .default(''),
  ),
  ...
  guardian_post_code: z.preprocess(
    ...
    z
      .string()
      .refine((val) => !val || POST_CODE.test(val), 'Post code must be 4 digits')
      .default(''),
  ),
  ...
  section: z
    ...
    .default(''),
  roll: z.string().min(1, 'Roll is required').regex(ROLL_NUMBER, 'Invalid roll').default(''),

  prev_school_name: z
    .string()
    .min(1, 'Previous School Name is required')
    .regex(ADDRESS_TEXT, 'Only English letters, digits and spaces are allowed for school name')
    .default(''),
  ...
  prev_school_upazila: z
    ...
    .regex(ADDRESS_TEXT, 'Only English letters, digits and spaces are allowed')
    .default(''),
  nearby_student_info: z.string().min(1, 'Nearby student information is required').default(''),
  photo: z.preprocess(
...
export type Class6RegistrationSettingsData = z.infer<typeof class6RegistrationSettingsSchema>;
export type RegistrationNoticeUploadData = z.infer<typeof registrationNoticeUploadSchema>;
export type RegistrationPhotoUploadData = z.infer<typeof registrationPhotoUploadSchema>;
