...
import { FiTrash2, FiEdit, FiX, FiCheck } from "react-icons/fi";
...
function AddLevel() {
  ...
  const fetchAssignedLevels = async () => {
    try {
      ...
      // toast.error("Failed to fetch class assignments");
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(
          `/api/level/updateLevel/${editingId}`,
          formData
        );
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `/api/level/deleteLevel/${levelToDelete}`
      );
                  ...
                  <tr key={level.id} className="">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm ">
                        {teachers.find((t) => t.id === level.teacher_id)
                          ?.name || "Not assigned"}
                      </div>
                    </td>
