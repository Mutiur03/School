...
const Admission = () => {
  ...
  const fetchSSCReg = async () => {
    try {
      ...
      if (error.response?.status !== 404) {
        setFormMessage("Error fetching admission data");
      }
      setIsEdit(false);
    } finally {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        setFormMessage("Admission info saved successfully!");
        setIsEdit(true);
        setNoticeFile(null);
      ...
      setFormMessage(
        error.response?.data?.message || "Error saving admission info"
      );
      setTimeout(() => setFormMessage(""), 3000);
    } finally {
      setFormLoading(false);
  ...
  const removeNotice = async () => {
    try {
      setFormLoading(true);
      const response = await axios.delete("/api/admission/");
  ...
  const handleRefresh = () => {
    // fetchAllRegistrations may be disabled in this build; only refresh SSC settings
    try {
      fetchSSCReg();
    } catch (e) {
      console.error("Error refreshing admission data:", e);
    }
  };
        ...
        <div className="flex flex-col md:flex-row justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              Configure settings and monitor student admission
            </p>
          </div>
        </div>
